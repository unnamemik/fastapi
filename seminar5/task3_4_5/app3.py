...
from task3_4_5.models import Users, UsersBase, Base, engine, db3
...
async def read_root():
    ...
    for user in users:
        res.append(f"  user_id: {user.user_id}, name: {user.name}, email: {user.email}")
    return res
...
async def create_item(user_id: int, user: Users):
    ...
    for user in users:
        ...
        user = UsersBase(user_id=user.user_id, name=user.name, email=user.email, password=user.password, is_del=False)
        db3.add(user)
        db3.commit()
        return f"User: user_id: {user.user_id}, name: {user.name}, email: {user.email}  "
